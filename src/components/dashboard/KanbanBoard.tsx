...
function groupKeyOf(status: string): string {
  const g = KANBAN_GROUPS.find(gr => gr.statuses.includes(status))
  return g ? g.key : KANBAN_GROUPS[0].key
}
...
export function KanbanBoard() {
  ...
  async function handleDrop(groupKey: string, e: React.DragEvent) {
    ...
    if (!task) return
    const group = KANBAN_GROUPS.find(g => g.key === groupKey)!
    if (group.statuses.includes(task.status)) return // ya está en este grupo
  ...
  return (
    <div className="flex gap-3 overflow-x-auto pb-3">
      {KANBAN_GROUPS.map(group => {
        const items = active.filter(t => groupKeyOf(t.status) === group.key)
        ...
        return (
          <div
            ...
            className={`flex-1 min-w-[230px] rounded-xl border transition-colors ${
          ...
          >
            ...
            <div className="flex flex-col gap-1.5 p-2 min-h-[140px]">
              ...
              {!items.length && <div className="text-center py-6 text-gray-300 text-[11px]">—</div>}
            </div>
          </div>
        )
      })}
